...
const modifyPermissionMiddleware = async(req,res,next)=>
{
    ...
    // {
    //     const permissions = await Permission.findOne({userId:req.body.restrictedUserId});
    //     req.
    //     return next();
    // } 
    ...
    try {
        ...
        // check branch
        if(!(req.user.role === 'admin')) if(restrictUser.branch !== req.user.branch) return res.status(500).json({success:false,message:"user don't have permission to restrict givin user."})
